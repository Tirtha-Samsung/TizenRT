/****************************************************************************
 * arch/arm/src/armv7-a/up_gicv2_dump.c
 *
...
static inline void up_gic_dump_cpu(bool all, int irq, int nlines)
{
...
static void up_gic_dumpregs(uintptr_t regaddr, int nlines, int incr)
{
  unsigned int i;
...
static inline void up_gic_dump4(const char *name, uintptr_t regaddr,
                                 int nlines)
{
  irqinfo("       %s[%08lx]\n", name, (unsigned long)regaddr);
  up_gic_dumpregs(regaddr, nlines, 4);
}
...
static inline void up_gic_dump8(const char *name, uintptr_t regaddr,
                                 int nlines)
{
  irqinfo("       %s[%08lx]\n", name, (unsigned long)regaddr);
  up_gic_dumpregs(regaddr, nlines, 8);
}
...
static inline void up_gic_dump16(const char *name, uintptr_t regaddr,
                                  int nlines)
{
  irqinfo("       %s[%08lx]\n", name, (unsigned long)regaddr);
  up_gic_dumpregs(regaddr, nlines, 16);
}
...
static inline void up_gic_dump32(const char *name, uintptr_t regaddr,
                                  int nlines)
{
  irqinfo("       %s[%08lx]\n", name, (unsigned long)regaddr);
  up_gic_dumpregs(regaddr, nlines, 32);
}

/****************************************************************************
 * Name: up_gic_dump_distributor
 *
 * Description:
...
static inline void up_gic_dump_distributor(bool all, int irq, int nlines)
{
  ...
  if (all)
    {
      up_gic_dump32("ISR",  GIC_ICDISR(0),  nlines);
      up_gic_dump32("ISER/ICER", GIC_ICDISER(0), nlines);
      up_gic_dump32("ISPR/ICPR", GIC_ICDISPR(0), nlines);
      up_gic_dump32("SAR/CAR", GIC_ICDSAR(0), nlines);
      up_gic_dump4("IPR", GIC_ICDIPR(0), nlines);
      up_gic_dump4("IPTR", GIC_ICDIPTR(0), nlines);
      up_gic_dump16("ICFR", GIC_ICDICFR(0), nlines);
      up_gic_dump32("PPSIR/SPISR", GIC_ICDPPISR, nlines);
      up_gic_dump32("NSACR", GIC_ICDNSACR(0), nlines);
      up_gic_dump8("SCPR/SSPR", GIC_ICDSCPR(0), nlines);
    }
  else
    {
...
{
  unsigned int nlines = up_gic_nlines();
  ...
  up_gic_dump_cpu(all, irq, nlines);
  up_gic_dump_distributor(all, irq, nlines);
}
