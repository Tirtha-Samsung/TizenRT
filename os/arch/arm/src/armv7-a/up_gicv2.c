/****************************************************************************
 * arch/arm/src/armv7-a/up_gicv2.c
 *
...
void up_gic0_initialize(void)
{
  ...
  up_gic_dump("Entry up_gic0_initialize", true, 0);
  ...
  DEBUGVERIFY(irq_attach(GIC_IRQ_SGI1, up_start_handler, NULL));
  DEBUGVERIFY(irq_attach(GIC_IRQ_SGI2, up_pause_handler, NULL));
#endif

  up_gic_dump("Exit up_gic0_initialize", true, 0);
}
...
void up_gic_initialize(void)
{
  ...
  up_gic_dump("Entry up_gic_initialize", true, 0);
...
#  warning Missing logic
  ...
  up_gic_dump("Exit up_gic_initialize", true, 0);
}
  ...
  if (irq > GIC_IRQ_SGI15 && irq < NR_IRQS)
    {
      ...
      up_gic_dump("Exit up_enable_irq", false, irq);
    }
}
  ...
  if (irq > GIC_IRQ_SGI15 && irq < NR_IRQS)
    {
      ...
      up_gic_dump("Exit up_disable_irq", false, irq);
    }
}
  ...
  if (irq >= 0 && irq < NR_IRQS)
    {
      ...
      up_gic_dump("Exit up_prioritize_irq", false, irq);
      return OK;
    }
