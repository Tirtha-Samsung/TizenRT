...
static int qemu_pl011_attach(struct uart_dev_s *dev)
{
  ...
  //Kishore gic_irq_set_priority(sport->irq_num, IRQ_TYPE_LEVEL, 0);
...
void up_serialinit(void)
{
  ...
  if (ret < 0)
    {
      dbg("error at register dev/console, ret =%d\n", ret);
    }
  ...
  if (ret < 0)
    {
      dbg("error at register dev/ttyS0, ret =%d\n", ret);
    }
}
